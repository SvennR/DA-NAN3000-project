#!/bin/sh
# from https://github.com/dtschan/curl-static
echo "******* Building static curl binary *******"
mkdir curl-static-build
cd curl-static-build
set -e
...
cat <<EOF | sudo docker run -i --rm -v "$PWD":/out --tmpfs /tmp/build:exec -w /tmp/build alpine /bin/sh -eus
...
if [ -f "curl" ]; then
    ...
    echo "Verify that curl is statically built.."
    #file curl  # is actually claims to be dynamically linked
                # but ldd confirms that it isnt linked to anything  
    ldd curl
    echo "Copying curl binary to projectroot/bin-static/"
    if [ ! -d "../bin-static" ]; then
      mkdir ../bin-static
    fi
    cp curl ../bin-static
    ls -la ../bin-static | grep "curl"
    echo "Deleting curl-static-build directory.."
    cd ..
    rm -rf curl-static-build 
    echo "Done!"
    return 0
else
    echo "Build failed.."
